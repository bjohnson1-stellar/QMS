...
def export_lookups(
    ...
    if output_path is None:
        ...
        if configured:
            ...
            # Default: next to the data directory
    ...
    # Build workbook
    wb = Workbook()
    # Remove the default blank sheet after we add ours
    ...
    for sheet_name, columns, rows in sheet_data:
        ...
        # Format as an Excel Table so Power Automate can use
        # "List rows present in a table" action
        if rows:
            last_col = get_column_letter(len(columns))
            last_row = len(rows) + 1
            table_ref = f"A1:{last_col}{last_row}"
            # Table name must be alphanumeric (no spaces)
